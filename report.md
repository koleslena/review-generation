...
- В обоих случаях не получаются осмысленные отзывы. Необходима дальнейшее обучение моделей, модели очевидно недообучены. Модель с мешком слов лучше справляется с заданием, так как в сгенерированных ею отзывах есть осмысленные фразы, хотя она обучалась всего на 0,5% от всех данных. Есть предположение, что если бы хватило ресурсов обучить ее хотя бы на больше 50% всех данных, то результаты были бы очень хорошими. Так как объем словаря для "мешка слов" очень большой, то чтобы модель хорошо справлялась с задачей нужно обучать на большом объеме данных, и нужны большие вычислительные ресурсы.



## Общие выводы

- Модель на основе уже обученной модели RuGPT очевидно показывает лучший результат. Модели построенные для генерации отзывов с нуля требуют очень больших ресурсов для обучения даже на таких сравнительно небольших данных. Модель с методом "мешок слов" имеет очень большой словарь, чтобы нейросеть работала хорошо, необходим очень большой объем данных. Для обучения такой сложной модели с нуля необходимы большие ресурсы. 
